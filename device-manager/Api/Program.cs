...
app.MapPut("/api/devices/{id}", async (DfaDeviceManagerContext context, int id, CreateDeviceDto dto, HttpContext http) =>
{
    if (string.IsNullOrWhiteSpace(dto.Name) ||
        string.IsNullOrWhiteSpace(dto.DeviceTypeName) ||
        dto.AdditionalProperties.ValueKind == JsonValueKind.Undefined)
    ...
    else if (role == "User")
    {
        var empIdClaim = http.User.FindFirstValue(ClaimTypes.NameIdentifier);
        if (!int.TryParse(empIdClaim, out var empId)) return Results.Forbid();
        var assignment = await context.DeviceEmployees
            .FirstOrDefaultAsync(de => de.DeviceId == id && de.EmployeeId == empId && de.ReturnDate == null);
